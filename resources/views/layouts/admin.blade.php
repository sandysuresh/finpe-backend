        ...
        <div class="mx-auto flex h-[70px] max-w-[1600px] items-center px-5">
            ...
            <nav class="hidden flex-1 items-center gap-1 xl:flex">
                ...
                @foreach ($menus as [$label, $url, $dropdown])
                    <a href="{{ $url }}" class="flex items-center gap-1.5 rounded-lg px-3 py-2.5 text-[13px] font-semibold {{ $label === 'Dashboard' ? 'bg-blue-50 text-blue-700' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900' }}">
                        {{ $label }}
                        @if($dropdown)
                ...
                <div class="group relative">
                    <button class="flex items-center gap-2.5 rounded-lg px-1.5 py-1 hover:bg-slate-50">
                        ...
                        <span class="hidden text-left lg:block">
                            <span class="block text-[13px] font-semibold text-slate-800">{{ auth('admin')->user()->name }}</span>
                            <span class="block text-[11px] text-slate-500">{{ ucfirst(auth('admin')->user()->role) }}</span>
                        </span>
        ...
        <div class="border-t border-slate-100 xl:hidden">
            <nav class="fi-scroll flex gap-1 overflow-x-auto px-4 py-2">
                @foreach ($menus as [$label, $url])
                    <a href="{{ $url }}" class="whitespace-nowrap rounded-lg px-3 py-2 text-xs font-semibold {{ $label === 'Dashboard' ? 'bg-blue-50 text-blue-700' : 'text-slate-600 hover:bg-slate-50' }}">{{ $label }}</a>
                @endforeach
            </nav>
        </div>
    </header>
